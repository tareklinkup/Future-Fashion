const salesInvoice = Vue.component('sales-invoice', {
        ...
        <div>
            <div class="row">
                <div class="col-xs-12">
                    ...
                    <label for="arabic" style="margin-left: 5px;">
                        <input type="radio" id="arabic" value="arabic" v-model="language"> Arabic
                    </label>
                </div>
            </div>
            ...
            <div id="invoiceContent">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-5 text-right">
                        ...
                        <strong>{{ language == 'english' ? 'Sales Date' : language == 'bangla' ? 'তারিখ' : 'تاريخ المبيعات'}}: </strong> {{ sales.SaleMaster_SaleDate }} {{ sales.AddTime | formatDateTime('h:mm a') }}
                    </div>
                </div>
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <table _a584de>
                            ...
                            <tbody>
                                <tr v-for="(product, sl) in cart">
                                    ...
                                    <td>{{ product.quantity_display_text }}</td>
    ...
    methods:{
        ...
        convertNumberToWords(amountToWord) {
            var words = new Array();
            words[0] = '';
            words[1] = this.language == 'english' ? 'One' : this.language == 'bangla' ? 'এক' : 'واحد';
            words[2] = this.language == 'english' ? 'Two' : this.language == 'bangla' ? 'দুই' : 'إثنان';
            words[3] = this.language == 'english' ? 'Three' : this.language == 'bangla' ? 'তিন' : 'ثلاثة';
            words[4] = this.language == 'english' ? 'Four' : this.language == 'bangla' ? 'চার' : 'أربع';
            words[5] = this.language == 'english' ? 'Five' : this.language == 'bangla' ? 'পাঁচ' : 'خمسة';
            words[6] = this.language == 'english' ? 'Six' : this.language == 'bangla' ? 'ছয়' : 'ستة';
            words[7] = this.language == 'english' ? 'Seven' : this.language == 'bangla' ? 'সাত' : 'سبعة';
            words[8] = this.language == 'english' ? 'Eight' : this.language == 'bangla' ? 'আট' : 'ثمانية';
            words[9] = this.language == 'english' ? 'Nine' : this.language == 'bangla' ? 'নয়' : 'تسعة';
            words[10] = this.language == 'english' ? 'Ten' : this.language == 'bangla' ? 'দশ' : 'عشرة';
            words[11] = this.language == 'english' ? 'Eleven' : this.language == 'bangla' ? 'এগারো' : 'أحد عشر';
            words[12] = this.language == 'english' ? 'Twelve' : this.language == 'bangla' ? 'বারো' : 'اثني عشر';
            words[13] = this.language == 'english' ? 'Thirteen' : this.language == 'bangla' ? 'তেরো' : 'ثلاثة عشر';
            words[14] = this.language == 'english' ? 'Fourteen' : this.language == 'bangla' ? 'চৌদ্দ' : 'أربعة عشرة';
            words[15] = this.language == 'english' ? 'Fifteen' : this.language == 'bangla' ? 'পনেরো' : 'خمسة عشر';
            words[16] = this.language == 'english' ? 'Sixteen' : this.language == 'bangla' ? 'ষোলো' : 'السادس عشر';
            words[17] = this.language == 'english' ? 'Seventeen' : this.language == 'bangla' ? 'সতেরো' : 'سبعة عشر';
            words[18] = this.language == 'english' ? 'Eighteen' : this.language == 'bangla' ? 'আঠারো' : 'الثامنة عشر';
            words[19] = this.language == 'english' ? 'Nineteen' : this.language == 'bangla' ? 'উনিশ' : 'تسعة عشر';
            words[20] = this.language == 'english' ? 'Twenty' : this.language == 'bangla' ? 'বিশ' : 'عشرين';
            words[30] = this.language == 'english' ? 'Thirty' : this.language == 'bangla' ? 'ত্রিশ' : 'ثلاثون';
            words[40] = this.language == 'english' ? 'Forty' : this.language == 'bangla' ? 'চল্লিশ' : 'أربعين';
            words[50] = this.language == 'english' ? 'Fifty' : this.language == 'bangla' ? 'পঞ্চাশ' : 'خمسون';
            words[60] = this.language == 'english' ? 'Sixty' : this.language == 'bangla' ? 'ষাইট' : 'ستون';
            words[70] = this.language == 'english' ? 'Seventy' : this.language == 'bangla' ? 'সত্তর' : 'سبعون';
            words[80] = this.language == 'english' ? 'Eighty' : this.language == 'bangla' ? 'আশি' : 'ثمانون';
            words[90] = this.language == 'english' ? 'Ninety' : this.language == 'bangla' ? 'নব্বই' : 'تسعين';
            ...
            if (n_length <= 9) {
                ...
                for (var i = 0; i < 9; i++) {
                    ...
                    if ((i == 1 && value != 0) || (i == 0 && value != 0 && n_array[i + 1] == 0)) {
                        words_string += this.language == 'english' ? "Crores " : this.language == 'bangla' ? " কোটি " : " كرور ";
                    }
                    if ((i == 3 && value != 0) || (i == 2 && value != 0 && n_array[i + 1] == 0)) {
                        words_string += this.language == 'english' ? "Lakhs " : this.language == 'bangla' ? " লক্ষ " : " كهس ";
                    }
                    if ((i == 5 && value != 0) || (i == 4 && value != 0 && n_array[i + 1] == 0)) {
                        words_string += this.language == 'english' ? "Thousand " : this.language == 'bangla' ? " হাজার " : " ألف ";
                    }
                    if (i == 6 && value != 0 && (n_array[i + 1] != 0 && n_array[i + 2] != 0)) {
                        words_string += this.language == 'english' ? "Hundred and " : this.language == 'bangla' ? " শত এবং " : " مائة و ";
                    } else if (i == 6 && value != 0) {
                        words_string += this.language == 'english' ? "Hundred " : this.language == 'bangla' ? " শত " : " مائة ";
                    }
                }
                words_string = words_string.split("  ").join(" ");
            }
            return words_string + (this.language == 'english' ? ' only' : this.language == 'bangla' ? ' টাকা মাত্র' : ' فقط');
        },
        async print(){
